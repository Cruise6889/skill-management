...
- 为本机 Skill 关联或重新关联原始目录，预览后手动同步；
- 手动检查 GitHub 默认分支并拉取选定 Skill 子目录的更新；
- 将两个 Skill 的八类规则拆解结果并排比较；
- 在线编辑文本文件，保存前查看逐行差异；
- 每次导入、来源更新、编辑和恢复都会留下完整历史版本；
- 通过文件级与逐行 Diff 检查变更，并查看文件引用图；
- 安装或导出前选择目标目录、预览冲突，可保留两份或在备份后覆盖。
